...
def retail():
    ...
    def upload_to_gcs(src_path, dst_path, task_id):
        try:
            ...
            logger.info(f"Successfully uploaded {src_path} to {dst_path} in bucket {bucket_name}")
        except Exception as e:
            logger.error(f"Failed to upload {src_path} to {dst_path} in bucket {bucket_name}: {e}")
